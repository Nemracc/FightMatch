...
<div class="card mt-5">
  <h2 class="card-header">Editar Peleador</h2>
  <div class="card-body">
    ...
    <form action="{{ route('peleadores.update', $peleador) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select 
                ...
                <option value="M" {{ $peleador->sexo == 'M' ? 'selected' : '' }}>Masculino</option>
                <option value="F" {{ $peleador->sexo == 'F' ? 'selected' : '' }}>Femenino</option>
            </select>
            @error('sexo')
                <div class="form-text text-danger">{{ $message }}</div>
            @enderror
